...
По условию нам необходимо, чтобы шар перемещался между кубами разного цвета. Такое можно реализовать, если отслеживать координаты каждого куба и различать их по тегу или имени. Был выбран вариант различия по имени, также добавлено несколько флажков в скрипт RollerAgent для создания небольшой логики.
...
```
Знаячения в конфиге тренера не изменялись, поэтому обучение затянулось. По итогу получилось следующее:

![Норм_Trim](https://user-images.githubusercontent.com/90757310/197833063-2855a3b6-9e91-42d9-bcea-e90a4c3f299e.gif)

## Выводы
В ходе лабораторной работы был обучен ML-агент для реализации ИИ в среде Unity. Для обучения использовались библиотеки mlagents 0.28.0 и torch версии 1.7.1 PyTorch отвечает за вычислительные функции в процессе обучения, поскольку этот фреймворм имеет тензорную направленность, то для наших задач является наиболее удобной. 
Для выполнения задания 2 была изучена документация по созданию тренера агента, что пригодится в процессе выполнения задания 3. Изучение компонентов Behavior Parametrs, отвечающий за поведение агента и сбор данных для последующего принятия решений через Decision Requester.
Для задания 3 необходимо было научить агент различать игровые объекты. Цвет придает им Материал, взаимодействие с которым наглядно, достаточно сложно. Поэтому был выбран подход инкапсуляции тега GameObject/имени GameObject. Далее в скрипт была добавлена логика вознаграждения при условии, что тег или имя объекта не совпадало с последним пораженным объектом.
## Powered by
